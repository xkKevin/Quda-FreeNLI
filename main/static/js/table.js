...
TINY.table=function(){
	...
	sorter.prototype.init=function(){
		...
		for(i;i<t.w;i++){
			...
			if(d){
			    var o = document.createElement('option');
			    o.value = i; o.innerHTML = c.innerHTML;
                d.appendChild(o)
			}
		}
		this.reset()
	...
	sorter.prototype.size=function(s){
		...
		if(this.p.navid){T$(this.p.navid).style.display=this.d<2?'none':'inline-block'}
		this.page(0);
	...
	function cp(f,c){
		...
		let i,n;
		if (reg.test(f) && reg.test(c)){
			i=parseFloat(f); n=parseFloat(c);
			g=i; h=n
